...
from .utiles import trunc_normal_
...
class PSyncBatchNorm(nn.SyncBatchNorm):
    def __init__(self,
                 *args,
                 bunch_size,
                 **kwargs):
        procs_per_bunch = min(bunch_size, utiles.get_world_size())
        assert utiles.get_world_size() % procs_per_bunch == 0
        n_bunch = utiles.get_world_size() // procs_per_bunch
        #
        ranks = list(range(utiles.get_world_size()))
        print('---ALL RANKS----\n{}'.format(ranks))
        ...
        bunch_id = utiles.get_rank() // procs_per_bunch
