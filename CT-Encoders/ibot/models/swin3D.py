...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.mlplayer0=MLPBlockSwin(48,384) #384, 256 CHANGE
        self.mlplayer1=MLPBlockSwin(96,384)
        self.mlplayer2=MLPBlockSwin(192,384)
        self.mlplayer3=MLPBlockSwin(384,384)
    ...
    def forward(self, x, normalize=True, return_all_tokens=None, mask=None):
        ...
        if return_all_tokens:
            ...
            #print("x_out_all=",torch.cat([x.unsqueeze(1), x_region], dim=1).shape)
            return torch.cat([x.unsqueeze(1), x_region], dim=1)
        #print(x0_out.shape,x1_out.shape,x2_out.shape,x3_out.shape)
        x0_out=self.mlplayer0(x0_out)
        x1_out=self.mlplayer1(x1_out)
        x2_out=self.mlplayer2(x2_out)
        x3_out=self.mlplayer3(x3_out)
        #print(x0_out.shape,x1_out.shape,x2_out.shape,x3_out.shape)
        x_out=torch.concatenate((x1_out,x2_out,x3_out),axis=1)#x0_out,
        #print("x_out=",x_out.shape)

        x_region = self.norm(x_out)  # B L C
        x = self.avgpool(x_region.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)
        return torch.cat([x.unsqueeze(1), x_region], dim=1)
...
def swin_3D(patch_size=(4,4,4),window_size=(7, 7, 7),**kwargs):
