...
from loader import CTFolderMask, PETCTFolderMask3D, HistoricalDocFolderMask, PETCTFolderMask3D_MLNS
...
def get_args_parser():
    ...
    parser.add_argument('--type_dataset', default="nrrd", type=str, help="""Type of dataset.""")
    ...
    parser.add_argument('--csv_path', default='', type=str,
        help='Please specify path to the csv for swin3d training data.')
    parser.add_argument('--mode', default='ct', type=str,
        help='Please specify between "pet", "ct" or "ct_chest".')
...
def train_ibot(args):
    ...
    if args.arch == "swin3D":
        if args.type_dataset== "nrrd":
            dataset = PETCTFolderMask3D(
                ...
                batch_size=256)
        elif args.type_dataset== "dcm":
            dataset = PETCTFolderMask3D_MLNS(
                args.data_path, 
                args.csv_path, 
                args.mode, 
                transform=transform,
    ...
    if args.arch == "vit_lora":
        ...
        student = swin_3D(patch_size=(args.patch_size,args.patch_size,args.patch_size),
            #return_all_tokens=True, 
            masked_im_modeling=args.use_masked_im_modeling)
        
        teacher = swin_3D(patch_size=(args.patch_size,args.patch_size,args.patch_size)#,return_all_tokens=True
        )
    ...
    fp16_scaler = None
    if args.use_fp16:
        fp16_scaler = torch.amp.GradScaler(device)
...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
    ...
    for name_q, param_q in student.named_parameters(): #add student.module
    ...
    for it, (images, masks,patient_id) in enumerate(metric_logger.log_every(data_loader, 20, header)):
        ...
        masks = [msk.to(device,non_blocking=True) for msk in masks]        
        ...
        with torch.amp.autocast(device):
            ...
            #for i in teacher_output:
            #    print("teacher_output final size:" ,i.shape)
            #for i in student_output:
            #    print("student_output final size:" ,i.shape)
                
            # get local views
            student.backbone.masked_im_modeling = False #add student.module
            #print("largo imagenes= ",len(images))
            student_local_cls = student(images[args.global_crops_number:])[0] if len(images) > args.global_crops_number else None
            student.backbone.masked_im_modeling = args.use_masked_im_modeling #add student.module
...
class iBOTLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, student_local_cls, student_mask, epoch):
        ...
        #print(np.shape(student_cls),np.shape(teacher_cls))
        if student_local_cls is not None:
            student_cls = torch.cat([student_cls, student_local_cls])
        #print("student_cls shape",len(student_cls))
        ...
        #print("teacher_cls_c shape",len(teacher_cls_c))
        #print("student_cls_c shape",len(student_cls_c))
        
        total_loss1, n_loss_terms1 = 0, 0
        total_loss2, n_loss_terms2 = 0, 0
        for q in range(len(teacher_cls_c)):
            for v in range(len(student_cls_c)):
                #print("ts_size:",teacher_patch_c[q].shape,student_patch_c[v].shape)
                if v == q:
                    loss2 = torch.sum(-teacher_patch_c[q] * F.log_softmax(student_patch_c[v], dim=-1), dim=-1)
                    #print("teacher_patch_c=",teacher_patch_c[v].shape,"student_patch_c=",student_patch_c[v].shape)
                    #print("student_mask_shape=",student_mask[v].shape)
                    if args.arch == 'swin3D':
                        #print("WASSSSSSAAAAAAAAAAAAA")
                        s_mask = student_mask[v]
                        mask = []
                        #mask.append(s_mask.repeat_interleave(8, -3).repeat_interleave(8, -2).repeat_interleave(8, -1).flatten(1))
                        mask.append(s_mask.repeat_interleave(4, -3).repeat_interleave(4, -2).repeat_interleave(4, -1).flatten(1))
                        mask.append(s_mask.repeat_interleave(2, -3).repeat_interleave(2, -2).repeat_interleave(2, -1).flatten(1))
                        mask.append(s_mask.flatten(1))
                        mask = torch.cat(mask, dim=-1)
                    else:
                        mask = student_mask[v].flatten(1)
                    #print("loss_shape=",loss2.shape,"mask_shape=",mask.shape)
...
class DataAugmentationiBOT(object):
    def __init__(self, global_crops_scale, local_crops_scale, global_crops_number, local_crops_number,mode):
        ...
        if mode=="ct":
            ...
            #normalize = T.Normalize(mean=[0.1218], std=[0.2063]) #CT NCLS
        else:
            normalize = T.Normalize(mean=[0.0660], std=[0.2874]) #PET NCLS
